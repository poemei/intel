...
static int contains_ci(const char *text,const char *needle){size_t n;const char *p;if(!text||!needle||!*needle)return 0;n=strlen(needle);for(p=text;*p;++p)if(_strnicmp(p,needle,n)==0)return 1;return 0;}
...
static int source_is(const rictus_intelligence_item_t *i,const char *const *sources,size_t count){size_t x;for(x=0;x<count;++x)if(_stricmp(i->source,sources[x])==0)return 1;return 0;}
...
int rictus_intelligence_relevance_evaluate(const rictus_intelligence_item_t *i,rictus_intelligence_relevance_result_t *r)
{
    ...
    first_party=_stricmp(i->source,"STN-LABZ Threat API")==0;
    if(!r->domain_flags||!material){r->relevance=RICTUS_INTELLIGENCE_RELEVANCE_NONE;strcpy_s(r->reason,sizeof(r->reason),!r->domain_flags?"No STN-LABZ engineering-domain consequence was established by retained source evidence.":"Engineering-domain vocabulary was present, but no material engineering consequence was established.");return 1;}
    if(generic&&!any(i,substantive,COUNT(substantive))){r->relevance=RICTUS_INTELLIGENCE_RELEVANCE_NONE;strcpy_s(r->reason,sizeof(r->reason),"Generic comment activity did not independently establish a material STN-LABZ engineering consequence.");return 1;}
    if(cyber_related&&!first_party&&!product_source&&!explicit_boundary){r->relevance=RICTUS_INTELLIGENCE_RELEVANCE_NONE;strcpy_s(r->reason,sizeof(r->reason),"Cybersecurity reporting named no explicit STN-LABZ technology, dependency, product, or owned-service boundary; generic vulnerability language is insufficient.");return 1;}
    r->relevance=RICTUS_INTELLIGENCE_RELEVANCE_RELEVANT;
    if(first_party)strcpy_s(r->reason,sizeof(r->reason),"First-party Sentinel evidence concerns an STN-LABZ-managed service; severity depends on the exact observed path and outcome.");
    else if(product_source||explicit_boundary)strcpy_s(r->reason,sizeof(r->reason),"The retained evidence explicitly intersects an approved STN-LABZ technology or product boundary; applicability beyond the cited evidence remains analytical.");
    else strcpy_s(r->reason,sizeof(r->reason),"The retained evidence identifies a material systems-engineering or mission condition within an STN-LABZ analytical domain.");
    return 1;
}
